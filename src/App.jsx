...
function buildAdj(vertices, edges, directed) {
  ...
  edges.forEach(({ from, to }) => {
    ...
    if (!directed && adj[to]) adj[to].push(from)
  })
  return adj
}
...
function fechoDireto(start, adj) {
  const reach = new Set(), queue = [start]
  while (queue.length) {
    const v = queue.shift()
    ;(adj[v] || []).forEach(n => { if (!reach.has(n)) { reach.add(n); queue.push(n) } })
  }
  reach.delete(start)
  return [...reach]
}
...
function isConnected(vertices, edges, directed) {
  ...
  if (!directed) {
    // Não-dirigido: basta um DFS cobrir todos
    const adj = buildAdj(vertices, edges, false)
    return dfs(vertices[0], adj).length === vertices.length
  }
  // Dirigido: fortemente conexo = todo vértice alcança todos os outros
  // Verifica DFS no grafo normal E no grafo transposto a partir do mesmo nó
...
function kosarajuSCC(vertices, edges) {
  ...
  function dfs1(v) {
    visited.add(v)
    ;(adj[v] || []).forEach(n => { if (!visited.has(n)) dfs1(n) })
    order.push(v)
  }
  vertices.forEach(v => { if (!visited.has(v)) dfs1(v) })
  const visited2 = new Set(), sccs = []
  function dfs2(v, comp) {
    visited2.add(v); comp.push(v)
    ;(radj[v] || []).forEach(n => { if (!visited2.has(n)) dfs2(n, comp) })
  }
...
export default function App() {
  ...
  const addEdge = () => {
    const f = eFrom.trim().toUpperCase(), t = eTo.trim().toUpperCase()
    if (!f||!t||f===t) return
    if (!vertices.includes(f)||!vertices.includes(t)) { setResult({ type:'error', msg:'Ambos os vértices devem existir.' }); return }
    if (edges.some(e => e.from===f&&e.to===t)) { setResult({ type:'error', msg:'Essa aresta já existe.' }); return }
    setEdges(p => [...p,{from:f,to:t}]); setEFrom(''); setETo('')
  ...
  const runDFS = () => {
    const start = percursoStart||vertices[0]; if(!start) return
  ...
  const runFecho = () => {
    const v = fechoVertex||vertices[0]; if(!v) return
    const adj = fechoType==='direto' ? buildAdj(vertices,edges,true) : buildReverseAdj(vertices,edges)
    const reach = fechoDireto(v,adj)
    setHighlight({ nodes:[v,...reach], selected:v })
    setResult({ type:'fecho', title:`Fecho ${fechoType} de ${v}`, data:reach })
  }
  ...
  const runAnalise = () => {
    ...
    setHighlight({})
    setResult({ type:'analise', conexo, sccs })
  }

  const handleNodeClick = v => {
    if (activeTab==='Arestas' && connectingFrom) {
      if (connectingFrom!==v && !edges.some(e=>e.from===connectingFrom&&e.to===v)) {
        setEdges(p=>[...p,{from:connectingFrom,to:v}])
        setResult({ type:'info', msg:`Aresta ${connectingFrom} ${directed?'→':'—'} ${v} adicionada.` })
      }
      setConnectingFrom(null); setHighlight({}); return
    }
    if (activeTab==='Arestas') {
      setConnectingFrom(v); setHighlight({selected:v})
      setResult({ type:'info', msg:`Clique em outro vértice para conectar com "${v}".` }); return
    }
  ...
  const handleCanvasClick = (x,y) => {
    if (activeTab==='Vértices') {
      const label = prompt('Nome do vértice:'); if(!label) return
  ...
  const edgesDisplay = directed ? edges : edges.filter((e,i) => !edges.slice(0,i).some(x=>x.from===e.to&&x.to===e.from))
    ...
    <div className="app">
      <header className="header">
        ...
        <div className="header-center">
          ...
          <button className={`toggle-btn ${directed?'active':''}`} onClick={()=>{setDirected(true);setEdges([]);clearHighlight()}}>Dirigido</button>
        </div>
      ...
      <div className="layout">
        <aside className="sidebar">
          <nav className="tabs">
            {TABS.map(t=>(
              <button key={t} className={`tab ${activeTab===t?'active':''}`} onClick={()=>{setActiveTab(t);clearHighlight()}}>{t}</button>
            ))}
          </nav>
          <div className="panel-content">
            ...
            {activeTab==='Vértices' && (
              <div className="section">
                ...
                <div className="input-row">
                  <input value={vInput} onChange={e=>setVInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addVertex()} placeholder="Nome (ex: A)" maxLength={4} className="inp"/>
            ...
            {activeTab==='Arestas' && (
              <div className="section">
                ...
                {connectingFrom && <div className="connecting-badge">De: <strong>{connectingFrom}</strong> → clique no destino</div>}
                <div className="input-row">
            ...
            {activeTab==='Análise' && (
              <div className="section">
                <p className="hint">Verifica conectividade e calcula os Componentes Fortemente Conexos (SCCs) pelo algoritmo de Kosaraju.</p>
        ...
        <main className="canvas-area">
          <GraphCanvas vertices={vertices} edges={edges} directed={directed} highlight={highlight}
        ...
        <aside className="result-panel">
          ...
          {result?.type==='analise' && (
            <div className="result-block">
              ...
              {!directed && !result.conexo && (
                <div>
                  <div className="result-subtitle" style={{marginTop:12}}>Componentes conexas:</div>
                  {(() => {
                    const adj = buildAdj(vertices,edges,false)
                    const seen = new Set(), comps = []
                    vertices.forEach(v => { if(!seen.has(v)) { const c=dfs(v,adj); c.forEach(x=>seen.add(x)); comps.push(c) } })
